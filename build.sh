...
docker run --rm \
  ...
  -v "${WORKDIR}/tmp/images:/images:rw" \
  -v "${WORKDIR}/tar:/tar:rw" \
  -it container /scripts/create-plain-image.sh "${IMAGE_SIZE}" "/images/${IMAGE_FILE}" "/tar/"
...
docker run --rm \
  -v "${WORKDIR}/scripts:/scripts:ro" \
  -v "${WORKDIR}/tmp/images:/images:rw" \
  -it container /scripts/encrypt-image.sh "${IMAGE_SIZE}" "/images/${IMAGE_FILE}" "/images/pass.txt"
